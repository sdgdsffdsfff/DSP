...
SERVER_NAME=`sed '/application.name/!d;s/.*=//' classes/system.properties | tr -d '\r'`
SERVER_PORT=`sed '/web_port/!d;s/.*=//' classes/system.properties | tr -d '\r'`
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.glsx.cloudframework.server.jetty.SpringJettyMain env=$ENV > $STDOUT_FILE 2>&1 &
