...
<@dwz.search_form action="${request.contextPath}/management/security/user/list">
	...
	<li>
	    <label>所在组织：</label>
	    <select id="select" style="height:22px;width:150px;" name="organizationId">
	        <option value ="">---全部---</option>
	        <#list organzations as organzation> 	           
	           <#if organizationId??&&organizationId==organzation.id>
                   <option value ="${organzation.id}" selected="selected">${organzation.name}</option>
               <#else>
                   <option value ="${organzation.id}">${organzation.name}</option>   
               </#if>
	        </#list> 
	    </select>
	</li>
...
<@dwz.layout_content>
	<@dwz.tool_bar>
		...
		<@shiro.hasPermission name="User:reset">
			<@dwz.tool_line/>
			<@dwz.tool_button content="重置密码" class="arrow_refresh" target="ajaxTodo" href="${request.contextPath}/management/security/user/reset/password/{slt_uid}" title="确认重置密码为888888?"/>
